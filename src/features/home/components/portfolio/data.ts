...
export const PROJECTS: ProjectDetail[] = [
  {
    ...
    works: [
      "AWS Elastic Beanstalk을 활용한 배포 환경 구축",
    ...
    issues: [
      {
        ...
        issue:
          "Django 서버 사이드 렌더링 방식에서 10,000건 이상의 데이터를 테이블로 출력하면, 템플릿 렌더링 과정에서 병목이 발생해 로딩 시간이 약 30초 이상 소요되는 문제가 있었습니다.",
        solve:
          "서버의 렌더링 부담을 줄이기 위해, 서버는 페이지 레이아웃만 렌더링하고 테이블 데이터는 JSON 형태로 응답하도록 변경했습니다. 이후 클라이언트에서 가상돔 기반의 Tabulator를 활용해 테이블을 렌더링하도록 변경했습니다.",
        result:
          "렌더링 부담을 클라이언트로 분산시켜 로딩 시간을 약 30초에서 1초 내외로 단축하고, 사용자 체감 성능을 크게 개선했습니다.",
      },
      {
        title: "바코드 입력 UX 개선",
        issue:
