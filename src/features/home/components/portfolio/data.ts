    ...
    issues: [
      {
        ...
        solve:
          "서버의 렌더링 부담을 줄이기 위해, 서버는 페이지 레이아웃만 렌더링하고 테이블 데이터는 JSON 응답으로 분리했습니다. 이후 클라이언트에서 가상 스크롤 방식의 Tabulator로 테이블을 렌더링하도록 변경했습니다.",
        result:
          "화면에 보이는 영역의 행만 실제 DOM으로 생성하기 때문에, 데이터가 늘어도 초기 렌더링 비용이 일정하게 유지되었고, 초기 로딩 시간을 약 30초에서 1초 내외로 단축하여 사용자 체감 성능을 개선할 수 있었습니다.",
      },
      {
        title: "바코드 입력 UX 개선",
        issue:
